...
plt.hist(df['Pram'], bins=10000, density=True ,alpha=0.7, color='blue')
...
for i in year_range:
    ...
    counts_min, bin_edges_min, _ = plt.hist(df[(df['year'] == i) & (df['rh'] <= 0.33)]['Pram'] ,bins=500, density=False ,alpha=0.7, color='blue', label='Perihelion')
    counts_max, bin_edges_max, _  = plt.hist(df[(df['year'] == i) & (df['rh'] >= 0.44)]['Pram'] ,bins=500, density=False ,alpha=0.7, color='red', label='Aphelion')
