...
def get_data(num):
    ...
    # print(df_total)
    extraction_day_hours = df_total[(df_total['min'] <= 0.47) & (df_total['min'] >= 0.0)][['year','day','hour','min']]
    extraction_day = np.array(df_total[(df_total['min'] <= 0.47) & (df_total['min'] >= 0.0)][['year','day']])
    ...
    df_all = df_all[(df_all['rh'] <= 0.47) & (df_all['rh'] >= 0.0)]

    print(df_all)
    df_all.to_csv(f'/Users/gordonlai/Documents/ICL/ICL_Y4/MSci_Mercury/msci_mercury_solarwind/mercury_data_{craft_num}_noperi.csv',index=False) #change path for different computer
