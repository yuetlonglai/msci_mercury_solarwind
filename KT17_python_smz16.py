...
class KT17_python:
    def __init__(self, Pram=15, distind=50.0, rh=0.39):
        '''
        *** Parameters v1.0 ***
        ...
        self.mu= 190.00 # dipole moment [nT Rp^3]
        self.tilt= 0.00*3.141592650/180.00 # dipole tilt angle [radians]
        self.pdip= 0.00*3.141592650/180.00 # dipole longitude [radians]
        self.rdx= 0.00 # dipole x offset
        self.rdy= 0.00 # dipole y offset
        self.rdz= 0.1960 # dipole z offset
        ...
        self.r0= 1.420 # distance from Mercury center to fitted sub-solar magnetopause [Rp]

        self.alfa= 0.50 # magnetopause flaring factor
        self.tamp1= 7.640 # tail disk current magnitude
        self.tamp2= 2.060 # harris sheet current magntidue
        self.d0 = 0.090 # half-width of current sheet in Z at inner edge of tail current [Rp]

        self.deltadx= 1.00 # expansion magnitudes of tail current sheet in x direction
        self.deltady= 0.10 # expansion magnitudes of tail current sheet in y direction

        self.scalex= 1.50 # e-folding distance for the sunward expansion of the harris sheet
        self.scaley= 9.00 # scale distance for the flankward expansion of the harris sheet

        self.zshift= 3.50 # location of image sheets for harris sheet

        self.mptol= 1.0e-3 # Tolerance for magnetopause encounter

        self.r_taildisk=[59048.35734,-135664.4246,-913.4507339, 209989.1008, -213142.9370, \
                    19.69235037, -18.16704312, 12.69175932, -14.13692134, 14.13449724, \
                    7.682813704, 9.663177797, 0.6465427021, 1.274059603, 1.280231032]

        self.r_dipshld=[7.792407683, 74.37654983, 4.119647072, -131.3308600, 546.6006311, \
        ...
        self.r_diskshld= [-398.4670279, -1143.001682, -1836.300383, -73.92180417, -326.3986853, \
        ...
        self.r_slabshld=[-91.67686636, -87.31240824, 251.8848107, 95.65629983, -80.96810700, \
        ...
        *** Variables ***
        ...
        self.rhel = rh # Heliocentric distance
        self.act = distind # Disturbance Index

            
        self.f=2.06873-0.00279*self.act
        self.B_eq=self.mu*10**(-9)
        ...
        self.scale_fac = 1.45/(((2*self.B_eq**2)/(self.mu0*14.3*10**(-9)))**(1/6))

        self.P_ram = Pram # solar wind ram pressure [nPa]
        self.rss=(((2*self.B_eq**2)/(self.mu0*self.P_ram*10**(-9)))**(1/6))*self.scale_fac
        self.tamp1=6.4950+0.0229*self.act
        self.tamp2=1.6245+0.0088*self.act
    ...
    '''
    def kt17_tailslab(self,xmsm,ymsm,zmsm):     
        
        d=self.d0+self.deltadx*np.exp(xmsm/self.scalex)+self.deltady*(ymsm/self.scaley)**2
        dddx=self.deltadx/self.scalex*np.exp(xmsm/self.scalex)
        zpzi=zmsm+self.zshift
        zmzi=zmsm-self.zshift
    ...
    def kt17_taildisk(self,xmsm,ymsm,zmsm):
        nr3=int(self.n_taildisk/3)
        f=self.r_taildisk[0:nr3]
        b=self.r_taildisk[nr3:2*nr3] 
        c=self.r_taildisk[2*nr3:3*nr3]
        ...
        z=zmsm*sc
        d0_sc=self.d0*sc
        deltadx_sc=self.deltadx*sc
        deltady_sc=self.deltady*sc
    ...
    def kt17_shield(self,n,r,x,y,z):
    ...
    def kt17_mpdist(self,mode,x,y,z):
        rho2=y**2+z**2
        r=np.sqrt(x**2+rho2)
        rho=np.sqrt(rho2)
        ...
        rm=self.r0/np.sqrt(self.alfa*(1.00+ct)) # standard form of shue et al.,1997, magnetopause model
        if rm < r:
            id_var = -1 # NOTE conversion from id to id_var
        fi = r - rm
        
        if mode != 0:
            drm_dt=0.250*rm**3/self.r0**2*st
            gradfir=1.00
            gradfit=-drm_dt/r
    ...
    def kt17_dipole(self,xmsm,ymsm,zmsm):
    ...
    def kt17_bfield(self,n,x_a,y_a,z_a):

        # initialize variables
        
        kappa=self.r0/self.rss
        kappa3=kappa**3
        ...
        for i in range(0,n):
            ...
            fi,gradfix,gradfiy,gradfiz,id_var,t = self.kt17_mpdist(0,x,y,z)
            noshield = 0
            if fi < self.mptol:
                id_var = 1
            if noshield == 1:
            ...
            if id_var == 1:
                ...
                bx_dip, by_dip, bz_dip = self.kt17_dipole(x,y,z)
                bx_shld, by_shld, bz_shld = self.kt17_shield(self.n_dipshld,self.r_dipshld,x,y,z)
                '''
                ...
                bx_tldsk, by_tldsk, bz_tldsk = self.kt17_taildisk(x,y,z)
                bx_dkshld, by_dkshld, bz_dkshld = self.kt17_shield(self.n_diskshld,self.r_diskshld,x,y,z)
                '''
                bx_dsk=tamp1*(bx_tldsk+bx_dkshld)
                by_dsk=tamp1*(by_tldsk+by_dkshld)
                bz_dsk=tamp1*(bz_tldsk+bz_dkshld)
                '''
                bx_dsk=self.tamp1*(bx_dkshld)
                by_dsk=self.tamp1*(by_dkshld)
                bz_dsk=self.tamp1*(bz_dkshld)
                '''
                ...
                bx_tlslb, by_tlslb, bz_tlslb = self.kt17_tailslab(x,y,z)
                bx_slbshld, by_slbshld, bz_slbshld = self.kt17_shield(self.n_slabshld,self.r_slabshld,x,y,z)
                '''
                bx_slb=tamp2*(bx_tlslb+bx_slbshld)
                by_slb=tamp2*(by_tlslb+by_slbshld)
                bz_slb=tamp2*(bz_tlslb+bz_slbshld)
                '''
                bx_slb=self.tamp2*(bx_slbshld)
                by_slb=self.tamp2*(by_slbshld)
                bz_slb=self.tamp2*(bz_slbshld)
                '''
